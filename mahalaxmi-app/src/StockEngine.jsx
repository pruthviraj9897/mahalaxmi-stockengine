...
import { Package, Truck, RotateCcw, Users, Boxes, LayoutGrid, Plus, Trash2, AlertCircle, CheckCircle2, FileText, Archive, Printer, Download, Upload, History, Pencil, X, Ban, Settings, LogOut, Menu } from "lucide-react";
  ...
  return (
    <div className="app-shell" style={styles.app}>
      ...
      <main className="main-content" style={styles.main}>
        ...
        {tab === "ledger" && <PartyLedger data={data} />}
      </main>
    </div>
  );
}
...
function computeInvoiceLines(data, partyId, billStartStr, billEndStr) {
  ...
  // order), then by date within an item. Broken-charge lines always sort last,
  // by date — verified against the saved Invoice Line Archive (e.g. invoice #5:
  // 31 item lines, then the Broken Charge line at Sr. 32).
  const itemIndex = new Map(data.items.map((it, idx) => [it.id, idx]));
  lines.sort((a, b) => {
    const aBroken = a.kind === "broken";
    const bBroken = b.kind === "broken";
  ...
  const serviceTotal = round2(lines.filter((l) => l.kind === "returned").reduce((s, l) => s + l.serviceCharge, 0));
  const brokenTotal = round2(lines.filter((l) => l.kind === "broken").reduce((s, l) => s + l.amount, 0));
  const additionalCharges = round2(serviceTotal + transportTotal);
            ...
            <Panel title={`Preview — ${result.lines.length} line${result.lines.length > 1 ? "s" : ""}`} hint={result.lines.length > 15 ? "Over 15 lines — will print as a continuation invoice" : undefined}>
              <Table
                ...
                rows={result.lines.map((l, i) => [
                  i + 1,
                  <span>{l.itemName} {l.returned && <em style={styles.tinyTag}>returned</em>} {l.broken && <em style={styles.tinyTag}>broken</em>}</span>,
                  l.qty,
              ...
              <div style={styles.totalsBox}>
                ...
                <TotalRow label="Service Charges" value={result.serviceTotal} />
                <TotalRow label="Transport Charge" value={result.transportTotal} />
                <TotalRow label="Additional Charges (Service + Transport)" value={result.additionalCharges} bold />
                {gst && gst.applicable ? (
                  <>
...
function InvoicePrintView({ data, invoice }) {
  ...
  const pages = [];
  for (let i = 0; i < invoice.lines.length; i += 15) pages.push(invoice.lines.slice(i, i + 15));

  return (
    <Panel title={`Invoice #${invoice.invoiceNo}`} hint={pages.length > 1 ? `${pages.length} pages` : undefined}>
      ...
      <div className="print-area">
        {pages.map((pageLines, pageIdx) => (
          <div key={pageIdx} className={pageIdx < pages.length - 1 ? "print-page-break" : ""} style={styles.invoiceSheet}>
            ...
            <div style={styles.invoiceMetaRow}>
              ...
              <span><strong>Invoice No.:</strong> {invoice.invoiceNo}{pages.length > 1 ? ` (Page ${pageIdx + 1}/${pages.length})` : ""}</span>
            ...
            {pageIdx === pages.length - 1 && (
              <div style={styles.totalsBox}>
                ...
                {invoice.brokenTotal > 0 && <TotalRow label="  — of which Broken Charges" value={invoice.brokenTotal} />}
                <TotalRow label="Additional Charges" value={invoice.additionalCharges} />
                {invoice.gst?.applicable ? (
                  <>
...
function PartyLedger({ data }) {
  ...
  const totals = useMemo(() => {
    ...
    return {
      invoiced: round2(partyInvoices.reduce((s, i) => s + i.netTotal, 0)),
        ...
        <>
          ...
          <div className="print-area">
            <div style={styles.invoiceLetterhead}>
              ...
              <div style={{ ...styles.invoiceTagline, fontWeight: 700, marginTop: 4 }}>PARTY LEDGER</div>
            </div>
            <div style={styles.invoiceMetaRow}>
              <span><strong>Party:</strong> {party.code} — {party.name}</span>
              <span><strong>Printed on:</strong> {fmtDateDisplay(new Date().toISOString().slice(0, 10))}</span>
            </div>
            ...
            <div style={styles.statRow}>
              ...
              <StatCard label="Broken charges (₹)" value={totals.brokenCharges.toFixed(2)} />
            </div>
