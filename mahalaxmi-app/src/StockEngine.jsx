...
function PartyLedger({ data, persist }) {
  const [partyId, setPartyId] = useState("");
  const rentedPrintRef = useRef(null);
  const timelinePrintRef = useRef(null);
  const [paymentForm, setPaymentForm] = useState(emptyPaymentForm());
  const party = data.parties.find((p) => p.id === partyId);

  // Clear the print-area class off both nodes once the print dialog closes,
  // so the next print starts clean.
  useEffect(() => {
    const clear = () => {
      rentedPrintRef.current?.classList.remove("print-area");
      timelinePrintRef.current?.classList.remove("print-area");
    };
  ...
  // Add the print-area class directly to the DOM node via ref, synchronously,
  // right before calling print(). Going through React state instead
  // (setPrintMode -> re-render -> class appears) races window.print(): the
  // re-render commit happens after this handler returns, so print() can
  // capture the DOM before the class was ever added — which is exactly why
  // Party Ledger printed blank while Invoice Archive (no state toggle,
  // class always present) worked fine. Direct ref mutation removes the race
  // entirely by mirroring the Invoice Archive pattern.
  const triggerPrint = (mode) => {
    const target = mode === "rented" ? rentedPrintRef.current : timelinePrintRef.current;
    const other = mode === "rented" ? timelinePrintRef.current : rentedPrintRef.current;
    if (!target) return;
    other?.classList.remove("print-area");
...
const globalCss = `
  ...
  * { box-sizing: border-box; }
  ...
  @media print {
    ...
    /* Mobile screens still match (max-width:768px) while printing, which
       keeps .sidebar as position:fixed and .mobile-topbar as position:sticky —
       force them out of the way explicitly rather than relying only on
       visibility:hidden, since a fixed/sticky ancestor can otherwise become
       the containing block for .print-area and misplace it off the page. */
    .sidebar, .mobile-topbar, .mobile-backdrop { display: none !important; }
