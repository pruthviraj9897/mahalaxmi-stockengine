...
function PartyLedger({ data, persist }) {
  ...
  // Which print preview is mounted below, mirroring Invoice Archive's
  // `selectedId` pattern: null | "rented" | "timeline". The preview is a
  // conditionally-MOUNTED component with "print-area" written directly in
  // its JSX (never toggled after the fact), so there's no race between a
  // class being added and window.print() capturing the DOM — the same
  // reason Invoice Archive's print view has always worked reliably on mobile.
  const [printMode, setPrintMode] = useState(null);
  const party = data.parties.find((p) => p.id === partyId);

  // reset the payment form and any open print preview whenever the selected party changes
  useEffect(() => {
      ...
      {party && (
        <>
          ...
          <Panel
            ...
            hint="Delivered − returned, per item"
            action={
              <button style={styles.ghostBtn} onClick={() => setPrintMode("rented")}>
          ...
          <Panel
            ...
            action={
              <button style={styles.ghostBtn} onClick={() => setPrintMode("timeline")}>
                <Printer size={13} /> Print Preview
              </button>
            }
          >
...
function PartyLedgerPrintView({ data, party, mode, rentedItems, timeline, onClose }) {
  const company = data.company || DEFAULT_COMPANY;
  const isRented = mode === "rented";

  return (
    <Panel title={isRented ? "Currently Rented — Print Preview" : "Timeline — Print Preview"}>
      ...
      <div className="print-area">
        <div className="invoice-sheet" style={styles.invoiceSheet}>
          <div style={styles.invoiceLetterhead}>
            <div style={styles.invoiceCompany}>{company.name}</div>
            <div style={styles.invoiceTagline}>{company.tagline}</div>
            <div style={styles.invoiceAddress}>{company.address} · {company.email}</div>
            <div style={{ ...styles.invoiceTagline, fontWeight: 700, marginTop: 4 }}>
              {isRented ? "CURRENTLY RENTED" : "TIMELINE"} — {party.code} — {party.name}
            </div>
          </div>
          {isRented ? (
            rentedItems.length === 0 ? (
