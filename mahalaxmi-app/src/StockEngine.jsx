      ...
      if (returnInWindow && deliveryDate <= billEnd && rl.qty > 0) {
        ...
        if (days > 0) {
          ...
          lines.push({
            ...
            serviceRate: item.serviceCharge,
      ...
      <div className="print-area">
        {pages.map((pageLines, pageIdx) => (
          <div key={pageIdx} className={pageIdx < pages.length - 1 ? "print-page-break" : ""} style={styles.invoiceSheet}>
            ...
            <Table
              cols={["Sr.", "Item", "Qty", "Rate/Ft/Day", "S.Date", "E.Date", "Days", "Amount", "Svc Rate", "Svc Qty", "Svc Amount"]}
              rows={pageLines.map((l, i) => [
                ...
                l.days,
                l.amount.toFixed(2),
                l.serviceCharge > 0 ? (l.feet ? `${l.serviceRate} × ${l.feet}ft` : l.serviceRate) : "—",
                l.serviceCharge > 0 ? l.qty : "—",
                l.serviceCharge > 0 ? l.serviceCharge.toFixed(2) : "—",
              ])}
            />
            {pageIdx === pages.length - 1 && (
              <div style={styles.totalsBox}>
                <TotalRow label="Item Rent Amount" value={invoice.itemRentTotal} />
                <TotalRow label="Service Charges (per item, see table)" value={invoice.serviceTotal} />
                <TotalRow label="Transport Charges (per delivery challan)" value={invoice.transportTotal} />
                {invoice.gst?.applicable ? (
                  <>
