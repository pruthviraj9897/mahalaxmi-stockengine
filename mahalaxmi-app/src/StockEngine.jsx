...
import { Package, Truck, RotateCcw, Users, Boxes, LayoutGrid, Plus, Trash2, AlertCircle, CheckCircle2, FileText, Archive, Printer, Download, Upload, History, Pencil, X, Ban, Settings, LogOut } from "lucide-react";
              ...
              return (
                <div key={idx} style={styles.lineRow}>
                  ...
                  >
                    ...
                    {options.map((o) => (
                      <option key={o.challanId} value={o.challanId}>#{o.challanNo} ({o.date}) — pending {o.pending}</option>
                    ))}
                  </select>
                  <input style={{ ...styles.input, flex: 1 }} type="number" placeholder="Qty" value={l.qty} onChange={(e) => setLine(idx, { qty: e.target.value })} />
      ...
      <div className="print-area">
        {pages.map((pageLines, pageIdx) => (
          <div key={pageIdx} className={pageIdx < pages.length - 1 ? "print-page-break" : ""} style={styles.invoiceSheet}>
            ...
            <div style={styles.invoiceMetaRow}>
              <span><strong>Billing Period:</strong> {invoice.billStart} → {invoice.billEnd}</span>
            </div>
            <Table
              cols={["Sr.", "Item", "Qty", "Rate/Ft/Day", "S.Date", "E.Date", "Days", "Amount"]}
  ...
  const timeline = useMemo(() => {
    ...
    for (const inv of data.invoices) {
      ...
      events.push({
        ...
        detail: `Billing period ${inv.billStart} → ${inv.billEnd}`,
        amount: inv.netTotal,
      });
    }
